...
│   ├── camera_widget.py # 카메라/이미지 업로드 위젯
│   ├── recommendation_widget.py # 추천 결과 표시
│   └── map_widget.py # 지도 및 맛집 표시 위젯
├── api/
│   ├── openai_vision.py # OpenAI Vision API 클라이언트
│   ├── openai_tools.py # OpenAI Tools/Functions 정의
│   └── naver_map_api.py # 네이버 지도 API 클라이언트
├── analyzer/
│   ├── food_analyzer.py # 음식 분석 엔진
│   ├── recommendation_engine.py # 추천 알고리즘
│   └── restaurant_finder.py # 맛집 검색 엔진
├── database/
│   ├── db_manager.py # SQLite DB 관리
│   └── food_log.db # 식사 기록 DB ├── utils/
│   ├── image_processor.py # 이미지 전처리
│   ├── location_utils.py # 위치 정보 처리
│   └── config.py # 설정 관리 ├── data/
│   ├── food_images/ # 촬영한 음식 사진들
│   ├── restaurant_cache/ # 맛집 정보 캐시
│   └── exports/ # 식단 분석 리포트
├── main.py # 애플리케이션 진입점
├── .env # API Keys (OpenAI, Naver)
